...
class Employee(db.Model):
    ...
    # Add this method to the Employee class
...
def is_available(self, date, shift):
        """Check if employee is available for a given date and shift"""
        # Check if employee is on time off
        time_off = TimeOffRequest.query.filter(
            TimeOffRequest.employee_id == self.id,
        ...
        if time_off:
            return False, f"Employee has approved time off from {time_off.start_date} to {time_off.end_date}"
        ...
        if availability:
            if not availability.start_time or not availability.end_time:
                return False, f"Employee is not available on {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][day_of_week]}s"
            
            # Check if shift hours fall within availability hours
            if shift.start_time < availability.start_time or shift.end_time > availability.end_time:
                return False, f"Shift hours ({shift.start_time.strftime('%H:%M')} - {shift.end_time.strftime('%H:%M')}) are outside employee's availability ({availability.start_time.strftime('%H:%M')} - {availability.end_time.strftime('%H:%M')})"
        
        # Check for conflicting shifts on the same day
        existing_shifts = ShiftAssignment.query.filter(
        ...
        for existing in existing_shifts:
            ...
            if (shift.start_time <= existing_shift.end_time and shift.end_time >= existing_shift.start_time):
                return False, f"Employee already has a shift ({existing_shift.name}) scheduled during this time"
...
# Add these model classes to your app.py file
...
class Shift(db.Model):
    ...
    company = db.relationship('Company', backref='shifts')
    ...
    def get_duration_hours(self):
        ...
        if self.start_time > self.end_time:  # Overnight shift
            # Calculate duration across midnight
            midnight = time(0, 0)
            first_part = timedelta(hours=24) - timedelta(hours=self.start_time.hour, minutes=self.start_time.minute)
            second_part = timedelta(hours=self.end_time.hour, minutes=self.end_time.minute)
            return (first_part + second_part).total_seconds() / 3600
        else:
            # Regular shift within same day
            return ((timedelta(hours=self.end_time.hour, minutes=self.end_time.minute) - 
                    timedelta(hours=self.start_time.hour, minutes=self.start_time.minute)).total_seconds() / 3600)
...
class ShiftAssignment(db.Model):
    ...
    # Add a unique constraint for employee on a specific date/shift
    __table_args__ = (db.UniqueConstraint('employee_id', 'shift_id', 'date', name='_employee_shift_date_uc'),)
    
    def __repr__(self):
        return f'<ShiftAssignment {self.employee.first_name} on {self.date}>'
    
    def get_timesheet(self):
        """Get the timesheet for this shift assignment if it exists"""
        shift_date = self.date
        shift = self.shift
        
        # Calculate start and end datetime for this shift
        if shift.start_time <= shift.end_time:
            ...
            shift_start = datetime.combine(shift_date, shift.start_time)
            shift_end = datetime.combine(shift_date + timedelta(days=1), shift.end_time)
        
        # Look for a timesheet that overlaps with this shift
        timesheet = Timesheet.query.filter(
            Timesheet.employee_id == self.employee_id,
            Timesheet.clock_in >= shift_start - timedelta(hours=1),  # Allow for 1 hour early clock-in
            Timesheet.clock_in <= shift_end,
            db.or_(
                Timesheet.clock_out == None,
                Timesheet.clock_out >= shift_start
            )
        ).first()
...
class EmployeeAvailability(db.Model):
    ...
    start_time = db.Column(db.Time, nullable=True)  # NULL means unavailable all day
    end_time = db.Column(db.Time, nullable=True)
    
    employee = db.relationship('Employee', backref='availability')
    
    __table_args__ = (db.UniqueConstraint('employee_id', 'day_of_week', name='_employee_day_uc'),)
    
    def __repr__(self):
        if self.start_time and self.end_time:
            return f'<Availability {self.employee.first_name} on day {self.day_of_week} from {self.start_time}-{self.end_time}>'
        else:
            return f'<Availability {self.employee.first_name} unavailable on day {self.day_of_week}>'
...
class TimeOffRequest(db.Model):
    ...
    notes = db.Column(db.Text)  # Admin/approval notes
    requested_on = db.Column(db.DateTime, default=datetime.now)
    ...
    approver = db.relationship('User', foreign_keys=[decided_by])
    
    def __repr__(self):
        return f'<TimeOffRequest {self.employee.first_name} from {self.start_date} to {self.end_date}>'
    
    def days_count(self):
        """Get the number of days in this time off request"""
...
class SchedulePeriod(db.Model):
    ...
    company = db.relationship('Company', backref='schedule_periods')
    creator = db.relationship('User', foreign_keys=[created_by])
    
    def __repr__(self):
        return f'<SchedulePeriod {self.name or self.start_date} to {self.end_date}>'
    
    def get_assignments(self):
        """Get all shift assignments within this period"""
        return ShiftAssignment.query.filter(
            ShiftAssignment.date >= self.start_date,
            ShiftAssignment.date <= self.end_date
        ).all()
    
    def get_assignments_by_employee(self, employee_id):
        """Get all shift assignments for a specific employee within this period"""
        return ShiftAssignment.query.filter(
            ShiftAssignment.employee_id == employee_id,
            ShiftAssignment.date >= self.start_date,
            ShiftAssignment.date <= self.end_date
        ).order_by(ShiftAssignment.date, ShiftAssignment.shift_id).all()

# Update Employee model with a method to check availability
...
def shifts():
    ...
    company = current_user.company
    shifts = Shift.query.filter_by(company_id=company.id).all()
...
def edit_shift(shift_id):
    ...
    # Make sure the shift belongs to the current user's company
    if shift.company_id != current_user.company.id:
        flash('Access denied.')
        return redirect(url_for('shifts'))
    ...
    if request.method == 'POST':
        ...
        start_time_str = request.form.get('start_time')
        end_time_str = request.form.get('end_time')
        shift.color = request.form.get('color')
        shift.is_night_shift = 'is_night_shift' in request.form
...
def delete_shift(shift_id):
    ...
    # Check if this shift is assigned to any employees
    assignments_count = ShiftAssignment.query.filter_by(shift_id=shift_id).count()
    if assignments_count > 0:
        flash(f'Cannot delete: This shift is assigned to {assignments_count} instances.')
        return redirect(url_for('shifts'))
...
def schedule():
    ...
    # Get date range parameters or default to current week
    ...
    shifts = Shift.query.filter_by(company_id=company.id, is_active=True).all()
    ...
    for employee in employees:
        ...
        shift_assignments[employee.id] = assignments
    
    # Get all time-off requests that overlap with this period
    ...
    time_off_days = {}
    for request in time_offs:
        current_date = max(request.start_date, start_date)
        end = min(request.end_date, end_date)
        
        while current_date <= end:
            date_key = current_date.strftime('%Y-%m-%d')
            if date_key not in time_off_days:
                time_off_days[date_key] = []
            
            time_off_days[date_key].append(request.employee_id)
            current_date += timedelta(days=1)
...
def assign_shift():
    ...
    try:
        ...
        # Verify employee and shift exist and belong to user's company
        employee = Employee.query.get_or_404(employee_id)
        shift = Shift.query.get_or_404(shift_id)
        
        if employee.company_id != current_user.company.id or shift.company_id != current_user.company.id:
            return jsonify({'success': False, 'message': 'Access denied'})
        
        # Check if employee is available
        is_available, message = employee.is_available(date, shift)
        if not is_available and 'already has a shift' in message:
            # If conflict is with another shift, delete the existing assignment
...
def remove_shift():
    ...
    try:
        ...
        # Make sure the assignment belongs to an employee in the current user's company
...
@app.route('/employee_availability/<int:employee_id>', methods=['GET', 'POST'])
@login_required
def employee_availability(employee_id):
        ...
        for day in range(7):  # 0=Monday to 6=Sunday
            ...
            if available and start_time_str and end_time_str:
                try:
                    ...
                    flash(f'Invalid time format for {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"][day]}.')
                    continue
            else:
                # Mark as unavailable
    ...
    # Get existing availability for each day
    availabilities = {}
    for day in range(7):
        availability = EmployeeAvailability.query.filter_by(
            employee_id=employee_id,
...
def time_off_requests():
    ...
    if current_user.is_admin:
        ...
        pending_requests = TimeOffRequest.query.join(Employee).filter(
            Employee.company_id == company.id,
            TimeOffRequest.status == 'pending'
        ).order_by(TimeOffRequest.start_date).all()
        
        approved_requests = TimeOffRequest.query.join(Employee).filter(
            Employee.company_id == company.id,
            TimeOffRequest.status == 'approved'
        ).order_by(TimeOffRequest.start_date).all()
        
        denied_requests = TimeOffRequest.query.join(Employee).filter(
            Employee.company_id == company.id,
        ...
        if not employee:
            flash('Your account is not linked to an employee record.')
            return redirect(url_for('dashboard'))
        ...
        approved_requests = TimeOffRequest.query.filter_by(
            employee_id=employee.id,
            status='approved'
        ).order_by(TimeOffRequest.start_date).all()
        
        denied_requests = TimeOffRequest.query.filter_by(
            employee_id=employee.id,
            status='denied'
        ).order_by(TimeOffRequest.start_date).all()
...
def request_time_off():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Time off request submitted successfully!')
            return redirect(url_for('time_off_requests'))
            
        except ValueError:
            flash('Invalid date format. Please use YYYY-MM-DD format.')
    
    return render_template('request_time_off.html', employee=employee)

@app.route('/approve_time_off/<int:request_id>')
@login_required
def approve_time_off(request_id):
    ...
    # Make sure the time off request belongs to an employee in the current user's company
    ...
    time_off.decided_by = current_user.id
    time_off.decided_on = datetime.now()
    
    db.session.commit()
    
    flash('Time off request approved!')
    return redirect(url_for('time_off_requests'))

@app.route('/deny_time_off/<int:request_id>', methods=['GET', 'POST'])
@login_required
def deny_time_off(request_id):
    ...
    if request.method == 'POST':
        ...
        flash('Time off request denied.')
        return redirect(url_for('time_off_requests'))
    
    return render_template('deny_time_off.html', time_off=time_off)

@app.route('/cancel_time_off/<int:request_id>')
@login_required
def cancel_time_off(request_id):
    time_off = TimeOffRequest.query.get_or_404(request_id)
    
    # Check if current user is admin or the employee who created the request
    employee = Employee.query.get(time_off.employee_id)
    is_owner = employee.email == current_user.email
    
    if not (current_user.is_admin or is_owner):
        flash('Access denied.')
        return redirect(url_for('dashboard'))
    
    # Additional check for admins - make sure it's their company
    if current_user.is_admin and employee.company_id != current_user.company.id:
        flash('Access denied.')
        return redirect(url_for('time_off_requests'))
    ...
    flash('Time off request cancelled.')
    return redirect(url_for('time_off_requests'))

# Schedule Periods Management
@app.route('/schedule_periods')
@login_required
def schedule_periods():
    ...
    company = current_user.company
    periods = SchedulePeriod.query.filter_by(company_id=company.id).order_by(SchedulePeriod.start_date.desc()).all()
    
    return render_template('schedule_periods.html', periods=periods)

@app.route('/create_schedule_period', methods=['GET', 'POST'])
@login_required
def create_schedule_period():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if end_date < start_date:
                flash('End date cannot be before start date.')
                return redirect(url_for('create_schedule_period'))
            ...
            flash('Schedule period created successfully!')
            return redirect(url_for('schedule_builder', period_id=period.id))
            
        except ValueError:
            flash('Invalid date format. Please use YYYY-MM-DD format.')
    
    # Default to next week if creating a new period
    today = datetime.now().date()
    next_monday = today + timedelta(days=(7 - today.weekday()))
    next_sunday = next_monday + timedelta(days=6)
    
    return render_template('create_schedule_period.html', 
                          default_start=next_monday.strftime('%Y-%m-%d'),
                          default_end=next_sunday.strftime('%Y-%m-%d'))

@app.route('/schedule_builder/<int:period_id>')
@login_required
def schedule_builder(period_id):
    if not current_user.is_admin:
        flash('Access denied.')
        return redirect(url_for('dashboard'))
    ...
    # Make sure the period belongs to the current user's company
    if period.company_id != current_user.company.id:
        flash('Access denied.')
        return redirect(url_for('schedule_periods'))
    ...
    for employee in employees:
        assignments = ShiftAssignment.query.filter(
            ShiftAssignment.employee_id == employee.id,
            ShiftAssignment.date >= period.start_date,
            ShiftAssignment.date <= period.end_date
        ).all()
    ...
    time_offs = TimeOffRequest.query.filter(
        TimeOffRequest.status == 'approved',
        TimeOffRequest.start_date <= period.end_date,
        TimeOffRequest.end_date >= period.start_date
    ).all()
    
    # Format for calendar display
    time_off_days = {}
    for request in time_offs:
        current_date = max(request.start_date, period.start_date)
        end = min(request.end_date, period.end_date)
...
@app.route('/publish_schedule/<int:period_id>')
@login_required
def publish_schedule(period_id):
    if not current_user.is_admin:
        flash('Access denied.')
        return redirect(url_for('dashboard'))
    ...
    period.published_on = datetime.now()
    
    db.session.commit()
    
    flash('Schedule published successfully!')
    return redirect(url_for('schedule_periods'))
